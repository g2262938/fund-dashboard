...
def run_backtest(
    ...
    for entry_idx in entry_indices:
        ...
        for pick in selected:
            trade_type = "strong" if pick["mode"] == "🟢 强势" else "steady"
            record = simulate_trade(pick, hold_periods, trade_type=trade_type)
...
def simulate_trade(pick: dict, hold_periods: tuple, trade_type: str = "strong") -> dict:
    """模拟单笔交易, 检查止盈止损
    trade_type: 'strong' 用 -7% 止损, 'steady' 用 -5% 止损 (更严格)
    """
    ...
    # ── v7 稳健股改动 ────────────────────────────────────────
    # 数据发现：稳健赢家100%靠移动止盈离场，输家74%被-5%直接斩
    # → 稳健硬止损从-5%改为-3%（防崩盘，但给趋势股足够空间）
    # → 同时移动止盈门槛从-10%回撤改为-7%（更早锁定利润）
    if trade_type == "steady":
        stop_loss_pct = 0.03  # 稳健: -3% 止损（原-5%太紧）
        trailing_trigger = 0.07  # 稳健: 从高点回撤7%触发止盈（原-10%太迟）
    else:
        stop_loss_pct = 0.07  # 强势: -7% 止损
        trailing_trigger = 0.10  # 强势: 从高点回撤10%触发止盈
    ...
    for i in range(entry_idx + 1, min(entry_idx + 1 + hold_max, len(kline))):
        ...
        # 检查硬止损 (盘中触发, v6: 🟡用-5%更严格)
        loss_pct = (lowest - entry_price) / entry_price * 100
        if loss_pct <= -stop_loss_pct * 100:
            sold = True
            sold_price = entry_price * (1 - stop_loss_pct)
            sold_date = bar["date"]
            stop_label = "(-7%)" if trade_type == "strong" else "(-3%,🟡)"
            sold_reason = f"🛑硬止损{stop_label}, 当日最低{lowest:.2f}"
            break

        # 跟踪最高价
        highest = high_so_far

        # ── v7: 内联移动止盈（trade_type 感知） ─────────────
        # 稳健: 从高点回撤 7% 卖一半，回撤 15% 全卖（让利润奔跑）
        # 强势: 从高点回撤 10% 卖一半，回撤 15% 全卖
        if trade_type == "steady":
            t_half = 0.07; t_full = 0.15
        else:
            t_half = 0.10; t_full = 0.15
        close = bar["close"]
        if highest > 0:
            retrace = (highest - close) / highest * 100
            if retrace >= t_full:
                sold = True
                sold_price = kline[i + 1]["open"] if i + 1 < len(kline) else close
                sold_date = kline[i + 1]["date"] if i + 1 < len(kline) else bar["date"]
                sold_reason = f"移动止盈全卖:从最高{highest:.2f}回撤{retrace:.1f}%"
                break
            elif retrace >= t_half:
                sold = True
                sold_price = kline[i + 1]["open"] if i + 1 < len(kline) else close
                sold_date = kline[i + 1]["date"] if i + 1 < len(kline) else bar["date"]
                sold_reason = f"移动止盈卖一半:回撤{retrace:.1f}%"
                break
